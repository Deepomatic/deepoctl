...
def argparser_init():
    ...
    input_groups = {
        cmd: subparsers_dict[cmd].add_argument_group('Input parameters')
        for cmd
        in ['infer', 'draw', 'blur', 'noop', 'add_images']
    }
    output_groups = {
        cmd: subparsers_dict[cmd].add_argument_group('Output parameters')
        for cmd
        in ['infer', 'draw', 'blur', 'noop']
    }
    model_groups = {
        cmd: subparsers_dict[cmd].add_argument_group('Model parameters')
        for cmd
        in ['infer', 'draw', 'blur']
    }
    onprem_groups = {
        cmd: subparsers_dict[cmd].add_argument_group('On-premises parameters')
        for cmd
        in ['infer', 'draw', 'blur']
    }
    option_groups = {
        cmd: subparsers_dict[cmd].add_argument_group('Option parameters')
        for cmd
        in ['infer', 'draw', 'blur', 'noop', 'add_images']
    }
    ...
    input_groups['add_images'].add_argument('-i', '--input', type=str, nargs='+', required=True,
    ...
    option_groups['add_images'].add_argument('--set_metadata_path', dest='set_metadata_path', action='store_true',
