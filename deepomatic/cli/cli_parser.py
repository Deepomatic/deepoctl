...
def argparser_init():
    ...
    for parser in [infer_parser, draw_parser, blur_parser]:
        ...
        parser.add_argument('-f', '--file', type=str, help="File that contains the workflows definitions")
        parser.add_argument('-t', '--threshold', type=float, help="Threshold above which a prediction is considered valid.", default=None)
        parser.add_argument('--input_fps', type=float, help="FPS used for input video frame skipping and extraction. If higher than the original video FPS, all frames will be analysed only once having the same effect as not using this parameter. If lower than the original video FPS, some frames will be discarded to simulate an input of the given FPS.", default=None)
        ...
        parser.add_argument('--output_fps', type=float, help="FPS used for output video reconstruction.", default=None)
        parser.add_argument('--inference_fps', type=float, help="FPS used for inference.", default=float('inf'))
